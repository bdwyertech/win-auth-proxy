...
func getAuthorizationHeader(proxyURL string) string {
	...
	// Acquire credentials
	cred, status, err := AcquireCredentials("dwyerb")
	if err != nil {
		log.Printf("AcquireCredentials failed: %v %s", err, auth_errors[int64(status)])
	}
	defer cred.Close()
	...
	log.Printf("NewContext success: status=0x%x errorcode=%s", status, auth_errors[int64(status)])
